...
# **2026-08-18 に1記事1本へ戻した**ので、どちらも「記事ごとに散らす」用途に変わった。
#   hashtag_sets: 記事の seed（categorySlug 由来）のハッシュで組を選ぶ。全ピンが同じタグに
#     なると §2-3 の固定率が上がるため。以前はバリアント v1/v2/v3 で切り替えていた。
#   taglines: フッタ帯は全ピンで100%一致していた（実測）ので同じく seed で回して固定率を下げる。
...
SITES = {
    "picknavi": {
        ...
        # ⚠️ 2026-08-18 に1記事1本へ戻したため、**この値は供給を上回っている**。
        # 記事の供給は DAILY_NEW_LIMIT=1（rakuten-affiliate-blog/scripts/config.py）＝週7本で、
        # ピンも週7本しか増えない。在庫42本は約3週で尽き、その後は週7本ペースになる。
        # 週14本を維持するなら記事の供給を1日2本に増やすか、この値を7に下げる（未決）。
        "weekly": 14,
